...
| Deployment artifact provenance | Partial | `adn-release-proof-v1` manifest finalization with registration response, invocation receipt, T3N evidence digest, replay proof, CI artifact evidence, remote GitHub artifact verification, and signed durable release asset verification |
...
- visible CI success exists for the release commit and `ci_release_sha.json` is generated by `.github/workflows/release-proof-attest.yml` after `.github/workflows/release-proof-input.yml` has completed successfully and uploaded the proof-input archive.
- `python scripts/verify_release_remote.py <proof-dir>` downloads the declared GitHub Actions artifact and verifies its digest and proof-input contents against the retained bundle.
- `python scripts/verify_release_asset.py <release-asset-dir>` verifies the signed published GitHub Release asset bundle after artifact retention expiry.
