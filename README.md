...
The bridge writes `proof/deployment_manifest_v3.9.2.local.json` first as a pending manifest with the actual post-build `localWasmSha256`, then finalizes it after registration with `registrationResponseDigest`, `registeredAt`, optional `contractId`, and the first validated invocation digest. This remains operator evidence rather than remote byte attestation until a pinned v3.9.2 proof bundle is committed.
