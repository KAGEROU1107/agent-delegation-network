...
console.log(`  ADN_BUILD_COMMIT=$BUILD_COMMIT ADN_RUSTC_VERSION="$RUSTC_VERSION" ADN_TRUSTED_ISSUER=${hex} cargo test --locked`);
console.log(`  ADN_BUILD_COMMIT=$BUILD_COMMIT ADN_RUSTC_VERSION="$RUSTC_VERSION" ADN_TRUSTED_ISSUER=${hex} cargo build --locked --target wasm32-wasip2 --release`);
console.log("");
console.log("Optional tenant-DID pin (use the DID printed in Phase 1):");
console.log(`  ADN_BUILD_COMMIT=$BUILD_COMMIT ADN_RUSTC_VERSION="$RUSTC_VERSION" ADN_TRUSTED_ISSUER=${hex} ADN_TENANT_DID=did:t3n:<tenant-hex> cargo test --locked`);
console.log(`  ADN_BUILD_COMMIT=$BUILD_COMMIT ADN_RUSTC_VERSION="$RUSTC_VERSION" ADN_TRUSTED_ISSUER=${hex} ADN_TENANT_DID=did:t3n:<tenant-hex> cargo build --locked --target wasm32-wasip2 --release`);
