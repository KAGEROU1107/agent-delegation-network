...
| Tests | Rust 21/21 (`cargo test --lib`); Python 34/34 (`pytest tests/negative_security.py`) |
...
- coordinator verifies each worker result (Ed25519 sig, signer, data_hash binding,
  delegation_id, audience, COMPLETED status, single-use result nonce) before consuming it
