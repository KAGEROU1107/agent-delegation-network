...
| Tests | Rust 25/25 (`cargo test --locked`, including digest delegation ID and pinned/unpinned production-path coverage); Python 70/70 (`pytest tests/negative_security.py tests/test_result_verifier.py tests/test_audit_guards.py`) |
...
python -m pytest tests/negative_security.py tests/test_result_verifier.py tests/test_audit_guards.py -v --tb=short   # 70/70
...
- worker request replay is recorded in a durable on-disk ledger keyed by
  ...
  active, and handler failures are marked `RETRYABLE_FAILURE` so the same
  authorization can be retried before expiry up to a bounded cap
