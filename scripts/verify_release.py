...
def verify_release_dir(proof_dir: Path | str) -> dict[str, Any]:
    ...
    if ci_release.get("attestation_phase") != "post_verify":
        raise RuntimeError("CI release evidence attestation_phase must be post_verify")
    ...
    if not str(ci_release.get("artifact_url", "")).startswith(expected_run_url_prefix):
        raise RuntimeError("CI release evidence artifact_url does not match expected repository")
